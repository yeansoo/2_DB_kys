...
-- SQL 작성법 : 대문자 작성 권장, 연결된 단어 사이에는 "_"(언더바) 사
...
COMMENT ON COLUMN "MEMBER".MEMBER_PWD IS '회원 비밀번'; 
...
INSERT INTO "MEMBER" VALUES('MEM01','123ABC','홍길동', '9911111-111111', DEFAULT);--SYSDATE
...
INSERT INTO "MEMBER" VALUES('MEM02','223ABC','김미미', '9911111-111113', SYSDATE);
INSERT INTO "MEMBER" VALUES('MEM03','223A','김라라', '9911111-111112', SYSDATE);
...
--INSERT INTO "MEMBER" VALUES('MEM05', 'PASS05','HEE', '000000-2222222',TO_DATE('2022-11-22')
-- 이부분 다시들
...
SELECT TABLESPACE_NAME, TABLE_NAME FROM ALL_ALL_TABLES WHERE TABLE_NAME LIKE '%EMPLOYEE%';
...
-- * 테이블 레벨 : 테이블 생성 시 컬럼 정의가 끝난 후 마지막에 생성 
...
--    USER_ID VARCHAR2(20) UNIQUE, -- 컬럼 레벨(제약조건명 미지정)
    --USER_ID VARCHAR(20) CONSTRAINT USER_ID_U UNIQUE,
    USER_ID VARCHAR(20),
    USER_PWD VARCHAR2(30) ,
    USER_NAME VARCHAR2(30),
    GENDER VARCHAR2(10),
...
--SQL Error [1] [23000]: ORA-00001: 무결성 제약 조건(KH_KYS.USER_ID_NAME_U)에 위배됩니다
...
-- NOT NULL + UNIQUE 제약조건의 의미 -> 중복되지않는 값이 필수로 존재해야함 
...
--> 기본키가 NULL 이므로 오류
-- ORA-01400: NULL을 ("KH_KYS"."USER_USED_PK"."USER_NO") 안에 삽입할 수 없습니다
...
-- 무결성 제약 조건(KH_KYS.PK_USERNO_USERID)에 위배됩니다
...
-- ORA-01400: NULL을 ("KH_KYS"."USER_USED_PK2"."USER_NO") 안에 삽입할 수 없습니다
...
CREATE TABLE USER_USED_FK(
  ...
  PHONE VARCHAR2(30),
  EMAIL VARCHAR2(50),
  GRADE_CODE NUMBER CONSTRAINT GRADE_CODE_FK REFERENCES USER_GRADE/*(GRADE_CODE)*/ -- 컬럼레
  --										-- 참조할 컬럼명(PK) 미작성 시 USER_GRADE의 PK를자동 참
  
  -- 테이블 레벨
--  , CONSTRAINT GRADE_CODE_FK FOREIGN KEY(GRADE_CODE) REFERENCES USER_GRADE 
				--> FORIGN KEY라는 단어는 테이블 레벨에서만 사용! 
  );
...
-- ORA-02290: 체크 제약조건(KH_KYS.GENDER_CHECK)이 위배되었습니다
...
--> 둘 다 칼럼의 값을 지정된 형태로만 제한을 하지만
-- CHECK는 적성 후 수정이 번거롭지만
-- FK는 부모 테이블에 행을 추가, 수정, 삭제 하는게 간편
...
CREATE TABLE USER_TEST(
	...
	UK_PNO CHAR(14) CONSTRAINT NN_PNO NOT NULL,
	GENDER CHAR(5) CONSTRAINT CK_GENDER CHECK(GENDER IN('남','여')),
	ADDRESS VARCHAR2(20),
	PHONE VARCHAR2(20),
	STATUS CHAR(1) CONSTRAINT STAT_YN NOT NULL ,
...
--> 서브쿼리로 테이블 생성시
-- 테이블의 형태(컬럼명, 데이터타입)만 복사되고
-- 제약조건, 코멘트는 복사되지 않기 때문에 별도의 추가작업이 필요하ㅇ다 
