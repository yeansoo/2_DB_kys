...
CREATE USER kys_sample IDENTIFIED BY sample1234;

-- 2. 새 연결 (접속 방법) 추가
--> ORA-01045: user KYS_SAMPLE lacks CREATE SESSION privilege; logon denied
									--> 접속 권한
...
GRANT CREATE SESSION TO kys_sample;
...
GRANT CREATE TABLE TO kys_sample;

ALTER USER kys_sample DEFAULT TABLESPACE
...
-- kh_kys	/	kys_sample 사용자 계정끼리 서로 객체 접근 권한 부여 

-- 1. (sample) kh_kys 계정의 EMPLOYEE 테이블 조회

SELECT * FROM KH_KYS.EMPLOYEE;

-- ORA-00942: 테이블 또는 뷰가 존재하지 않습니다
--> 접근 권한이 없어서 조회불가

-- 2. (kh) kys_sample 계정에 EMPLOYEE 테이블 조회 권한 부여

-- [객체 권한 부여 방법]
-- GRANT 객체권한 ON 객체명 TO 사용자명;

GRANT SELECT ON EMPLOYEE TO kys_sample;
...
-- 4. (sample) kh_kys.EMPLOYEE 테이블을 복사한 테이블을 생성
CREATE TABLE EMP_SAMPLE
AS SELECT * FROM KH_KYS.EMPLOYEE;
...
REVOKE SELECT ON EMPLOYEE FROM KYS_SAMPLE;
