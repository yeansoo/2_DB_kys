...
-- LENGTH(컬럼명 | 리터럴) : 길이 반환
SELECT EMAIL, LENGTH(EMAIL) 이메일길이
FROM EMPLOYEE;
...
SELECT EMP_NAME , SALARY, MOD(SALARY,100)
FROM EMPLOYEE ;
...
-- EMPLOYEE테이블에서 사번이 홀수인 사원의 사번, 이름 조회
SELECT EMP_ID, EMP_NAME
FROM EMPLOYEE 
WHERE MOD(EMP_ID,2)<>0;
...
SELECT ROUND(123.456,0) FROM DUAL; -- 123, 소수점 첫번째 자리에서 반올림(0기본값)
SELECT ROUND(123.456,-1) FROM DUAL; -- 123, 소수점 0번째 자리에서 반올림해서
								 	-- 소수점 -1자리 표현 == 1의자리에서 반올림해서 10의 자리부터 표현

SELECT ROUND(123.456,-2) FROM DUAL; -- 10의자리에서 반올림해서 100의 자리부터 표현
...
SELECT TRUNC(123.456,-1) FROM DUAL; -- 120 10의자리 아래 버림
...
SELECT SYSDATE FROM DUAL; --2022-09-01 11:21:45.000

-- 2022/09/01 11:21:45.000 목요일
...
SELECT SYSDATE FROM DUAL;
SELECT '2022-09-02'FROM DUAL;
SELECT 20220902 FROM DUAL;

SELECT TO_DATE('2022-09-02') FROM DUAL;-- DATE타입으로 변환
SELECT TO_DATE(20220902)  FROM DUAL;

SELECT '220901 113255' FROM DUAL;
SELECT TO_DATE('220901 113255','YYMMDD HH24MISS') 시간싱기 FROM DUAL;
--> 패턴을 적용해서 작성된 문자열의 각 문자가 어떤 날짜 형식인지 인지시킴

-- EMPLOYEE 테이블에서 각 사원이 태어난 생년월일(1990년 05월 13일) 조회
SELECT EMP_NAME , TO_CHAR(TO_DATE(SUBSTR(EMP_NO,1,6)),'YYYY"년" MM"월" DD"일"') -- INSTR(EMPNO,'-'-1)
FROM EMPLOYEE; 

-- Y패턴 : 현재 세기(21세기==20XX년==2000년대)
-- R패턴 : 1세기를 기준으로 절반(50년) 이상인경우 이전세기(1900년대)
--							절반(50년) 미만인경우 현재세기(2000년대)

SELECT TO_DATE('510505','YYMMDD') FROM DUAL; -- 2051-05-05 00:00:00.000
SELECT TO_DATE('510505','RRMMDD') FROM DUAL; --1949-05-05 00:00:00.000
