...
-- SYSDATE : 시스템상의 현재 시간(날씨) 나타내는 상수
...
SELECT SYSDATE-1 "하 루 전", SYSDATE AS 현재시간, SYSDATE +1 내일 
FROM DUAL;
...
----------------------------------------------------------------
-- DISTINCT : 조회시 컬럼에 표함된 중복값을 한 번만 표기
-- 주의사항 1) DISTINCT 구문은 SELECT 마다 딱 한 번 씩만 작성 가능하다. 
...
SELECT EMP_ID ,EMP_NAME ,SALARY,DEPT_CODE  FROM EMPLOYEE WHERE SALARY >2000000;
...
FROM EMPLOYEE 
WHERE SALARY < 30000000 OR  SAL_LEVEL > =50000000;
...
SELECT EMP_ID , EMP_NAME , SALARY , PHONE 
FROM EMPLOYEE 
WHERE SALARY BETWEEN  30000000 AND 60000000;
...
SELECT EMP_ID , EMP_NAME , SALARY , PHONE 
FROM EMPLOYEE 
WHERE SALARY NOT BETWEEN  30000000 AND 60000000;
...
--> _를 기준점으로 삼았으나 
-- 와일드카드 '_'와 동일한 표기법으로 작성되어 
-- 구분되지 않음

-- 해결방법 : LIKE의 ESCAPE OPTION을 이용하여 _를 구분한다. 

-- LIKE의 ESCAPE OPTION : '_' / '%' 앞에
